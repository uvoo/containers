...
[ CA_default ]
dir                       = ${PWD}/openssl_ca
# dir                       = /mnt/c/Users/jebusk/src/sandbox/containers/uvoo/ucert/openssl-ca/openssl_ca
crl_dir                   = \$dir/crl
database                  = \$dir/index
...
#             -extensions v3_ca \
openssl req -passin env:ROOTCA1_PWD \
            -key RootCA1.key \
            -new -x509 -days 3653 -sha256 -extensions v3_ca \
            -out RootCA1.crt -subj "${ROOTCA_SUBJ}"
...
# openssl req -passin env:ICA1a_PWD \
openssl req -passin env:ICA1a_PWD -config openssl.cnf \
            -new -sha256 \
            -key ICA1a.key \
            -out ICA1a.csr -subj "${ICA_SUBJ}"
# yes | openssl ca -passin env:ROOTCA1_PWD -config openssl.cnf \
           # -extensions v3_intermediate_ca \
# yes | openssl ca -passin env:ROOTCA1_PWD \
yes | openssl ca -passin env:ROOTCA1_PWD -config openssl.cnf \
           -extensions v3_intermediate_ca \
...
# openssl pkcs12 -export -passout env:ICA1A_PASSWORD -out ${CN_PREFIX}ICA1a.chain.pfx -passin env:ICA1A_PASSWORD -inkey ${CN_PREFIX}ICA1a.key -in ${CN_PREFIX}ICA1a.crt -certfile ${CN_PREFIX}ICA1a.chain

cat RootCA1.crt > ICA1a.chain
cat ICA1a.crt >> ICA1a.chain
openssl pkcs12 -export -passout env:ICA1a_PWD -out ICA1a.pfx -passin env:ICA1a_PWD -inkey ICA1a.key -in ICA1a.crt -certfile ICA1a.chain
