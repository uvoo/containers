...
func genTls(tlsFqdn string) {
	...
	if _, err := os.Stat("tls.key"); err != nil {
		...
		fmt.Printf("Create fqdn %s tls.crt and tls.key files.\n\n", tlsFqdn)
...
func serveHTTP(httpPort string, mux *http.ServeMux, errs chan<- error) {
	log.Printf("starting http server on port %v.", httpPort)
	errs <- http.ListenAndServe(":"+httpPort, mux)
}

func serveHTTPS(httpsPort string, mux *http.ServeMux, errs chan<- error) {
	cfg := &tls.Config{
...
func main() {
	...
	tlsFqdn := getEnv("HTTPS_TLS_FQDN", "demo.example.com")
	...
	go serveHTTP(httpPort, mux, errs)
	go serveHTTPS(httpsPort, mux, errs)
	log.Fatal(<-errs)
}
